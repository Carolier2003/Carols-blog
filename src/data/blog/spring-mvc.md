...
2. **路由匹配**：`HandlerMapping` 根据 URL 找到对应的 `Controller` 方法。

3. **方法执行**：`HandlerAdapter` 调用 `Controller` 方法，处理参数绑定和返回值。

4. **结果处理**：`Controller` 返回 `ModelAndView`（含数据 + 视图名）。

5. **视图解析**：`ViewResolver` 将逻辑视图名（如 `"success"`）解析为具体视图（如 `/WEB-INF/success.html`）。

6. **响应渲染**：`View` 将模型数据填充到视图模板，生成最终响应。
